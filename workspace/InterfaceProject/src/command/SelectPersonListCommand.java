...
public class SelectPersonListCommand extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// list에서 하나씩 데이터를 꺼낸다. -> 꺼낸 Person을 -> JSONObject로 바꿔준 뒤 -> JSONArray 추가	
		...
		for(Person person : list) {
			...
			obj.put("regdate", person.getRegdate().toString()); // 문자열로 바꿔주지않으면 regdate가 숫자로 인식되어 json데이터로 사용할 수 없어진다.
			arr.add(obj);
		}
