...
public class CaptchaImage {
	...
	// 기존 main() 메소드를
	// 캡차 이미지를 받아오는 getCaptchaImage() 메소드로 변환
	// 1. 반환타입 : void
	// 2. 메소드명 : getCaptchaImage
	// 3. 매개변수 
	//		1) HttpServletRequest request (realPath를 구하기 위해서 필요)
	//  	2) String key (발급받은 캡차 키)
    public static void getCaptchaImage(HttpServletRequest request, String key) {
    	
    	String clientId = "hJ0pKo3nC6k0fXDbsWoy"; //애플리케이션 클라이언트 아이디값";
        String clientSecret = "AajHMbo1df"; //애플리케이션 클라이언트 시크릿값";
    ...
    private static String getImage(HttpServletRequest request, InputStream is){
        ...
        try(OutputStream outputStream = new FileOutputStream(f)){
            ...
            // Login.java로 부터 받은 request를 사용하기 때문에
            // 여기에서 request에 데이터를 저장하면 Login.java도 저장된 데이터를 사용할 수 있다.
            // (Login.java의 request와 CaptchaImage.java의 request는 같은 것이기 때문에 CaptchaImage.java의 request에 저장한 데이터를 Login.java가 사용할 수 있다.)
